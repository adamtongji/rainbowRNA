...
outexprdir = args[1]
control_len =args[2]
control_len =as.numeric(control_len)
pairtype = args[3]
p_value = args[4]
setwd(outexprdir)
...
up<-diff[diff[,4]>0,]
down<-diff[diff[,4]<0,]
annot_table<-read.table('../results/annotation_table.txt',sep='\t')
annot_table2<-data.frame(circ=annot_table[,7],genes=annot_table[,5])
annot_table2<-subset(annot_table2,annot_table2[,1]!="n/a")
down_gene<-data.frame(circ=rownames(down))
down_gene2<-merge(down_gene,annot_table2,by="circ")
up_gene<-data.frame(circ=rownames(up))
up_gene2<-merge(up_gene,annot_table2,by="circ")

write.table(down_gene2$genes,file="../results/genes_down.txt",quote=F,row.names = F,col.names = F)
write.table(up_gene2$genes,file="../results/genes_up.txt",quote=F,row.names = F,col.names = F)
write.table(rownames(down),file="../results/circ_down.txt",quote=F,row.names = F,col.names = F)
write.table(rownames(up),file="../results/circ_up.txt",quote=F,row.names = F,col.names = F)

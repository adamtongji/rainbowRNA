...
#######################################################################
# Copyright(c) 2007-2010 Whitehead Institute for Biomedical Research.
#              All Rights Reserved
#
# Author: George Bell
...
#          It produces output as displayed in TargetScan Release 5.3
#
# This code is available from http://www.targetscan.org/cgi-bin/targetscan/data_download.cgi?db=worm_53
#
...
print COORDS "a_Gene_ID\tmiRNA_family_ID\tspecies_ID\tMSA_start\tMSA_end\tUTR_start\tUTR_end\tGroup_num\tSite_type\tmiRNA in this species\tGroup_type\tSpecies_in_this_group\tSpecies_in_this_group_with_this_site_type\n";
...
sub readMiRNAs
{
	...
	my ($MIR_FAM_ID, $mirSeedRegion, $mirSpeciesID);
	...
	while (<MIR_FAMILY_DATA>)
	{
		...
		($MIR_FAM_ID, $mirSeedRegion, $mirSpeciesID) = split (/\t/, $_);
...
sub summarizePrintGroupsThisGeneThisMiRNA
{
	...
	my ($uniqueSiteTypesList, $uniqueSiteTypesNamesList);
	%groupToInfo = ();
